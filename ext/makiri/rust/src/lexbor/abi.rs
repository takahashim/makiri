 ...
 * lowercases. `lxb_dom_element_qualified_name_set` does the same for an
 * element's name. */
extern "C" {
    ...
    /// Record an element's name as written beside its lower-cased tag - what
    /// the tree builder does for SVG's `linearGradient`, declaring it itself
    /// in `foreign_content.c`.
    pub fn lxb_dom_element_qualified_name_set(
