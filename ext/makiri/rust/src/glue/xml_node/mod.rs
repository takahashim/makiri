...
use self::abi::*;
use super::abi::{
    mkr_cDocument, mkr_cNodeSet, mkr_doc_parsed, mkr_parsed_xml_doc, NodeData,
};
...
pub unsafe extern "C" fn mkr_init_xml_node_read() {
    ...
    m.define_method("namespace_uri", method!(read::namespace_uri, 0)).expect("#namespace_uri");
    ...
    ns_class.define_method("prefix", method!(ns::ns_prefix, 0)).expect("Namespace#prefix");
    ns_class.define_method("href", method!(ns::ns_href, 0)).expect("Namespace#href");
    ns_class.define_method("to_s", method!(ns::ns_href, 0)).expect("Namespace#to_s");
    ns_class.define_method("==", method!(ns::ns_equal, 1)).expect("Namespace#==");
    ns_class.define_method("eql?", method!(ns::ns_equal, 1)).expect("Namespace#eql?");
    ns_class.define_method("hash", method!(ns::ns_hash, 0)).expect("Namespace#hash");
    ns_class.define_method("inspect", method!(ns::ns_inspect, 0)).expect("Namespace#inspect");

    m.define_method("namespace", method!(ns::namespace, 0)).expect("#namespace");
    m.define_method("namespace_definitions", method!(ns::namespace_definitions, 0))
        .expect("#namespace_definitions");
    m.define_method("namespaces", method!(ns::namespaces, 0)).expect("#namespaces");
    ...
    m.define_method("child", method!(read::first_child, 0)).expect("#child");
    ...
    let equals: RbMethod = core::mem::transmute(mkr_node_equals as unsafe extern "C" fn(VALUE, VALUE) -> VALUE);
    ...
    for name in ["external_id", "public_id"] {
        dt.define_method(name, method!(read::dtd_external_id, 0)).expect("#external_id");
    }
