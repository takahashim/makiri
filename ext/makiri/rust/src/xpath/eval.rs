 ...
 * the proof says it was written. A value comes back as an `Val`, so one
...
pub(crate) struct Memo(Vec<Option<Val>>);
...
unsafe fn string_value(bytes: &[u8], err: ErrSink, what: &core::ffi::CStr) -> EvalResult<Val> {
