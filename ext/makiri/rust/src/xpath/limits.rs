...
//!
//! Every overrun is ErrorKind::Limit - never a truncated or empty result.
...
fn over_ast_nodes(max: usize, err: ErrSink) -> Reported {
    err_setf!(err, ErrorKind::Limit, "AST node limit exceeded ({})", max)
}
...
fn over_check(max: usize, noun: &str, err: ErrSink) -> Reported {
    err_setf!(err, ErrorKind::Limit, "{} limit exceeded ({})", noun, max)
}
