...
#[cold]
#[inline(never)]
unsafe fn over_recursion(l: *mut Limits, err: *mut Error) -> c_int {
    err_setf!(
        err,
        XP_ERR_LIMIT,
...
#[cold]
#[inline(never)]
unsafe fn over_check(max: usize, noun: &str, err: *mut Error) -> c_int {
...
unsafe fn check(value: usize, max: usize, noun: &str, err: *mut Error) -> c_int {
    if value > max {
        return over_check(max, noun, err);
    }
    0
}
