...
fn lex_err(err: ErrSink, e: LexErr) -> Reported {
    match e {
        LexErr::ExpectedNumber => err_setf!(err, ErrorKind::Syntax, "expected number"),
        LexErr::UnterminatedString => {
            err_setf!(err, ErrorKind::Syntax, "unterminated string literal")
        }
        LexErr::InvalidUtf8Literal => {
            err_setf!(err, ErrorKind::Syntax, "invalid UTF-8 in string literal")
        }
        LexErr::UnexpectedChar(c) => {
            if (0x20..0x7F).contains(&c) {
                ...
                err_setf!(err, ErrorKind::Syntax, "unexpected byte 0x{:02x}", c)
            }
        }
    }
}
...
impl<'a> Parser<'a> {
    ...
    fn eat(&mut self, k: Tok, what: &str) -> PResult {
        if self.kind() != k {
            return Err(err_setf!(self.err, ErrorKind::Syntax, "expected {}", what));
        }
        self.advance()
    }
    ...
    fn fill_owned(&self, text: &[u8]) -> PResult<Box<[u8]>> {
        try_to_boxed_slice(text)
            .ok_or_else(|| err_setf!(self.err, ErrorKind::Oom, "out of memory in parser"))
    }
    ...
    fn parse_node_test(&mut self) -> PResult<NodeTest> {
        ...
        Err(err_setf!(self.err, ErrorKind::Syntax, "expected node test"))
    }
