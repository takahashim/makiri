/* mkr_xpath_shared_body.h — representation-independent engine primitives.
 *
 * Compiled exactly ONCE (via mkr_xpath_shared.c), with bare symbols, because
 * none of these functions dereferences a DOM node: they move node *pointers*
 * (node-set build/clone/free), own/compare engine strings, manage the per-eval
 ...
 *
 * The driver (mkr_xpath.c), the parser/lexer, AND both monomorphized engine
 * instances all call these by their bare names. The node-DEREFERENCING engine
 * code lives in the per-instance bodies (value/funcs/eval) and is compiled twice
 * with the MKR_NODE_* field-access bound to each representation.
 *
 * Three primitives here are extern rather than file-static (mkr_pointer_hash,
 * mkr_str_cache_index_put, mkr_str_cache_reindex): the string-value cache splits
 * its pure index bookkeeping (here) from its node-dereferencing insert
 * (mkr_get_cached_node_text, in the per-instance value body), so both sides
 * share the one index implementation.
 */
#ifndef MKR_DOM_NODE  /* this TU is representation-neutral: void node pointers */
#endif
