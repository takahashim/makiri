...
FORBID_FILES = %w[
  ...
  xpath/token.rs xpath/value.rs xpath/verify.rs
].freeze
...
# (`Lxb*`), the `lxb_*` functions and constants, and `crate::lexbor_abi` must not
# appear above it. The port is done, so the pinned table is empty and any name
# found outside fails; a file that needs one goes through a `lexbor` facade.
